...
def check_diagonal_lines_in_adjacent_squares(state, x, y, z):
    stones = state['stones']

    # Provera dijagonalne linije koja prolazi kroz susedne kvadrate za x i y
    if y == 0 or y == 2:
        if abs(sum(stones[i][0][z] for i in range(3))) == 3:
            return True
        if abs(sum(stones[i][2][z] for i in range(3))) == 3:
            return True
        # Dodatna provera za dijagonalne elemente
        if abs(sum(stones[i][y][z] for i in range(3))) == 3:
            return True
        # Dodatna provera za vertikalnu liniju između srednjeg i malog kvadrata
        if z == 0 and abs(sum(stones[i][y][1] for i in range(3))) == 3:
            return True
        if z == 2 and abs(sum(stones[i][y][1] for i in range(3))) == 3:
            return True

    # Provera dijagonalne linije koja prolazi kroz susedne kvadrate za x i z
    if z == 0 or z == 2:
        ...
        if abs(sum(stones[x][i][z] for i in range(3))) == 3:
            return True
        # Dodatna provera za horizontalnu liniju između srednjeg i malog kvadrata
        if y == 0 and abs(sum(stones[x][i][1] for i in range(3))) == 3:
            return True
        if y == 2 and abs(sum(stones[x][i][1] for i in range(3))) == 3:
            return True

    # Provera dijagonalne linije koja prolazi kroz susedne kvadrate za y i z
    if x == 0 or x == 2:
        if abs(sum(stones[x][i][0] for i in range(3))) == 3:
            return True
        if abs(sum(stones[x][i][2] for i in range(3))) == 3:
            return True
        # Dodatna provera za dijagonalne elemente
        if abs(sum(stones[x][y][i] for i in range(3))) == 3:
            return True
...
def is_making_line(state, move):
    ...
    if y == 0 or y == 2:
        ...
        if check_diagonal_lines_in_adjacent_squares(state, x, y, z):
            return True
    return False
